...
def parse_args():
    ...
    parser.add_argument('--path_connect', type=int, default=4, choices=[4, 8], help='the size of graph')
    # Define the tasks
    parser.add_argument('--task', choices=['Space', 'Time'], type=str,
                        default='Space', help='Spatial-based graph classification or Time-based graph classification')
...
if __name__ == '__main__':
    ...
    sub_dir = args.task + '_' + args.model_name + '_' + str(args.path_size[0]) + '-' + \
              str(args.path_size[1]) + '-' + str(args.path_connect) + '-' + datetime.strftime(
