...
class ChebyNet(torch.nn.Module):
    def __init__(self, feature, out_channel, pooltype='TopKPool'):
    ...
    def forward(self, data):
        ...
        x = self.bn1(x)
        x = F.relu(x)
        x, edge_index, batch = self.poolresult(self.pool1, self.pooltype, x, edge_index, batch)
        x1 = global_mean_pool(x, batch)

        x = self.GConv2(x, edge_index)
        x = self.bn2(x)
        x = F.relu(x)
        x, edge_index, batch = self.poolresult(self.pool2, self.pooltype, x, edge_index, batch)
